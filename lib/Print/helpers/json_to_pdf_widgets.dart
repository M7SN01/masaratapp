...
//, {Map<String, List<Map<String, dynamic>>> lists = const {}}
  ...
  switch (map['type']) {
    ...
    case 'table':
      ...
      //no need to pass headers as widget  only using header_list
      List<dynamic>? headerList = variables['header_list'];
      if (headerList != null) {
        headerList = headerList.reversed.toList();
      }

      return pw.TableHelper.fromTextArray(
        headers: headerList ?? headers.map((element) => interpolate(element['name'].toString())).toList(), // Make sure headers are ordered RTL manually
